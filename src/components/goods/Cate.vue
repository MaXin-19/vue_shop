<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 表格 :data为数据源（数组），:columns为表格各列  :selection-type是否为多选类型的表格，加：代表属性绑定-->
      <!-- :expand-type="false"是否为展开行类型的表格 show-index显示数据索引  index-text="#"数据索引名称  border显示纵向边框  :show-row-hover鼠标悬停时是否高亮显示-->
      <tree-table class="treeTable" :data="catelist" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border :show-row-hover="false">
        <!-- 是否有效 template模板定义属性。表格内就定义了一个插槽，名字叫isok-->
...
export default {
  data() {
    return {
      ...
      // 总数据条数
      total: 0,
      // 为table指定列的定义，为数组。label为标题，prop为指定的具体值
      columns: [
        {
          // 当前列标题
          label: '分类名称',
